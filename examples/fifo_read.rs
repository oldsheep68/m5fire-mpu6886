...
fn main() -> ! {
    ...
    mpu.enable_fifo(true, true);


    loop {
        if let Ok(fifo_data) = mpu.read_fifo() {
            println!("fifo_data: {:?}", fifo_data);
            println!("fifo_data: {:?} {:?}", fifo_data[0], fifo_data[1]);
        } else {
            println!("fifo_data read error");
        }
        //let fifo_data = mpu.read_fifo().unwrap();
        //println!("fifo_data: {:?}", fifo_data);
